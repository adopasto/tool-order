"""Demo data - mirror src/seed.js. Vsetky kontakty su fiktivne (@example.com) -
pred ostrym nasadenim preprac."""
...
def seed_if_empty() -> bool:
    if db.get("SELECT COUNT(*) AS c FROM users")["c"] > 0:
        return False

    with db.transaction():
        # --- pouzivatelia (identita = e-mail, prihlasenie riesi Next.js/Entra) ---
        users = [
        ...
        for username, full_name, email, role, cc in users:
            db.run("""INSERT INTO users (username, full_name, email, role, cost_center)
                       VALUES (?,?,?,?,?)""",
                   (username, full_name, email, role, cc))

        # --- kategorie ---
        cat: dict[str, int] = {}
        for name, sort in [("Ručné náradie", 10), ("Spájkovanie", 20), ("ESD ochrana", 30),
                           ("Meradlá", 40), ("Spotrebný materiál", 50), ("OOPP", 60)]:
            cat[name] = db.run("INSERT INTO categories (name, sort_order) VALUES (?,?)",
                               (name, sort))["lastrowid"]
        ...
        it: dict[str, int] = {}
        for code, name, desc, cat_name, unit, rop, roq, loc, esd in items:
            it[code] = db.run("""INSERT INTO items
                (code, name, description, category_id, unit, reorder_point, reorder_qty, location, is_esd)
                VALUES (?,?,?,?,?,?,?,?,?)""",
                (code, name, desc, cat[cat_name], unit, rop, roq, loc, esd))["lastrowid"]
        ...
        for code, q in stavy.items():
            db.run("""INSERT INTO stock_moves (item_id, move_type, qty, user_id, note)
                       VALUES (?, 'INVENTURA', ?, 5, 'Počiatočný stav')""", (it[code], q))
        ...
        for code, name, desc, cat_name, lines in baliky:
            bid = db.run("INSERT INTO bundles (code, name, description, category_id) VALUES (?,?,?,?)",
                        (code, name, desc, cat[cat_name]))["lastrowid"]
            for icode, q in lines:
                db.run("INSERT INTO bundle_items (bundle_id, item_id, qty) VALUES (?,?,?)", (bid, it[icode], q))
        ...
        for name, ico, person, email, phone, web, addr, lead, note, lines in dod:
            sid = db.run("""INSERT INTO suppliers
                (name, ico, contact_person, email, phone, web, address, lead_time_days, note)
                VALUES (?,?,?,?,?,?,?,?,?)""",
                (name, ico, person, email, phone, web, addr, lead, note))["lastrowid"]
            for icode, sku, price, moq, prim in lines:
                db.run("""INSERT INTO supplier_items
                    (supplier_id, item_id, supplier_sku, price, min_order_qty, is_primary) VALUES (?,?,?,?,?,?)""",
                    (sid, it[icode], sku, price, moq, prim))
