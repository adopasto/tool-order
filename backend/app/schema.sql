...
-- Identitu rieši Next.js (Auth.js - Entra ID alebo dev e-mail bypass) a na
-- kazdom requeste ju posiela v hlavicke X-Neways-User; FastAPI podla nej len
-- najde pouzivatela e-mailom. Heslo sa tu neriesi vobec.
...
-- Kosik je viazany na pouzivatela, nie na anonymnu session cookie - inak by
-- si ho pri zdielanom prehliadaci/pocitaci mohli vidiet rozni ludia.
CREATE TABLE IF NOT EXISTS cart_items (
  id      INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  kind    TEXT NOT NULL CHECK (kind IN ('item','bundle')),
  item_id INTEGER NOT NULL,              -- id polozky alebo balika, podla kind
  qty     REAL NOT NULL,
...
-- ---------------------------------------------------------------------------
-- Cenove ponuky dodavatelov (evidencia dokladov, nie objednavanie).
-- System u dodavatela nic neobjednava - ponuky sluzia ako podklad pre nakup
-- a ako zdroj mnozstevnych cenovych pasiem.
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS quotes (
  ...
  supplier_id  INTEGER REFERENCES suppliers(id),
  issued_at    TEXT,                          -- datum vystavenia (RRRR-MM-DD)
  valid_until  TEXT,                          -- platnost do
  total_no_vat REAL,
  total_vat    REAL,
  currency     TEXT NOT NULL DEFAULT 'EUR',
  file_path    TEXT,                          -- nazov suboru v data/quotes
  note         TEXT,
  created_at   TEXT NOT NULL DEFAULT (datetime('now','localtime'))
);
CREATE UNIQUE INDEX IF NOT EXISTS ux_quote_no ON quotes(supplier_id, number);

-- Riadok ponuky. Jedna polozka moze mat viac riadkov - mnozstevne pasma.
CREATE TABLE IF NOT EXISTS quote_lines (
  ...
  supplier_sku TEXT,
  description  TEXT,
  min_qty      REAL NOT NULL DEFAULT 1,       -- od akeho mnozstva cena plati
  unit         TEXT,
  unit_price   REAL,
  note         TEXT
);
